	...
	metronome: {
		...
		onHit(target, source, effect) {
			const moves = this.dex.moves.all().filter(move => (
				...
				!move.realMove && !move.isZ && move.num < 1199 && !move.isMax &&
...
//touhoumon moves begin here
	...
	aromatherapy2: {
		...
		flags: {snatch: 1, distance: 1},
		onHit(target, source, move) {
			this.add('-activate', source, 'move: Aromatherapy 2');
			let success = false;
			const allies = [...target.side.pokemon, ...target.side.allySide?.pokemon || []];
			for (const ally of allies) {
				if (ally !== source && ((ally.hasAbility('sapsipper')) ||
	...
	attract2: {
		...
		condition: {
			...
			onStart(pokemon, source, effect) {
				...
				if (!this.runEvent('Attract 2', pokemon, source)) {
					this.debug('Attract event failed');
					return false;
				}

				if (effect.name === 'Cute Charm') {
					this.add('-start', pokemon, 'Attract 2', '[from] ability: Cute Charm', '[of] ' + source);
				} else if (effect.name === 'Destiny Knot') {
					this.add('-start', pokemon, 'Attract 2', '[from] item: Destiny Knot', '[of] ' + source);
				} else {
					this.add('-start', pokemon, 'Attract 2');
				}
			},
			onUpdate(pokemon) {
				if (this.effectState.source && !this.effectState.source.isActive && pokemon.volatiles['attract']) {
			...
			onBeforeMovePriority: 2,
			onBeforeMove(pokemon, target, move) {
				this.add('-activate', pokemon, 'move: Attract 2', '[of] ' + this.effectState.source);
				if (this.randomChance(1, 2)) {
					this.add('cant', pokemon, 'Attract 2');
					return false;
				}
			},
			onEnd(pokemon) {
	...
	bindingvoice: {
		...
		pp: 15,
		priority: 0,
		flags: {protect: 1, sound: 1, mirror: 1, distance: 1},
		secondary: {
			chance: 20,
			volatileStatus: 'flinch',
		},
	...
	charminglook: {
		...
		condition: {
			...
			onStart(target) {
				...
				this.add('-start', target, 'move: Charming Look');
			},
			onResidualOrder: 15,
			onEnd(target) {
				this.add('-end', target, 'move: Charming Look');
			},
			onDisableMove(pokemon) {
				for (const moveSlot of pokemon.moveSlots) {
					const move = this.dex.moves.get(moveSlot.id);
			...
			onBeforeMove(attacker, defender, move) {
				if (!move.isZ && !move.isMax && move.category === 'Status' && move.id !== 'mefirst') {
					this.add('cant', attacker, 'move: Charming Look', move);
					return false;
				}
			},
		},
	...
	metronome2: {
		...
		onHit(target, source, effect) {
			const moves = this.dex.moves.all().filter(move => (
				(![2, 4].includes(this.gen) || !source.moves.includes(move.id)) &&
				!move.realMove && !move.isZ && move.num > 1198 && !move.isMax &&
				(!move.isNonstandard || move.isNonstandard === 'Unobtainable') &&
				!effect.noMetronome!.includes(move.name)
			));
			let randomMove = '';
	...
	poisonsting2: {
		...
		pp: 30,
		priority: 0,
		flags: {contac: 1, protect: 1, mirror: 1},
		secondary: {
			chance: 30,
			status: 'psn',
		},
	...
	roost2: {
		...
		condition: {
			...
			onType(types, pokemon) {
				this.effectState.typeWas = types;
				return types.filter(type => type !== 'Aero');
			},
		},
		secondary: null,
		target: "self",
	...
	skyattack2: {
		...
		pp: 5,
		priority: 0,
		flags: {contact: 1, charge: 1, protect: 1, mirror: 1, distance: 1, nosleeptalk: 1, failinstruct: 1},
		critRatio: 2,
		onTryMove(attacker, defender, move) {
			if (attacker.removeVolatile(move.id)) {
				return;
	...
	skitterrush: {
		...
		onTry(source) {
			if (source.activeMoveActions > 1) {
				this.hint("Skitter Rush only works on your first turn out.");
				return false;
			}
		},
		secondary: null,
	...
	cheer: {
		...
		condition: {
			...
			onStart(target, source) {
				this.effectState.multiplier = 1.5;
				this.add('-singleturn', target, 'Cheer', '[of] ' + source);
			},
			onRestart(target, source) {
				this.effectState.multiplier *= 1.5;
				this.add('-singleturn', target, 'Cheer', '[of] ' + source);
			...
			onBasePowerPriority: 10,
			onBasePower(basePower) {
				this.debug('Boosting from Cheer: ' + this.effectState.multiplier);
				return this.chainModify(this.effectState.multiplier);
			},
		},
		secondary: null,
