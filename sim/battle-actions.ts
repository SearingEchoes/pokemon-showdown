...
export class BattleActions {
	 ...
	 */
	 getDamage(
		...
		if (this.battle.gen <= 4 && ['explosion', 'selfdestruct', 'explosion2', 'selfdestruct2', 'explode'].includes(move.id) && defenseStat === 'def') {
	...
	modifyDamage(
		...
		if (pokemon.status === 'frz' && move.category === 'Special' && !pokemon.hasAbility('unwavering')) {
			//if (this.battle.gen < 6 || move.id !== 'facade') {
				baseDamage = this.battle.modify(baseDamage, 0.5);
			//}
		}
